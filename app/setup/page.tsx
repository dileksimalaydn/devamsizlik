...
export default function SetupPage() {
  ...
  const handleSave = async () => {
    ...
    if (userError) {
      setSaving(false);
      return alert("Auth hata: " + userError.message);
    }
      ...
      <div className="mx-auto max-w-md px-4 pt-4">
        <div className="rounded-3xl bg-white p-4 shadow-sm border border-slate-100">
          ...
          <div className="mt-1 text-xs text-slate-600">
            Ders adı + gün + başlangıç saati + blok seç.
          </div>
          ...
          <div className="mt-4 space-y-3">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="w-full rounded-2xl bg-slate-900 py-3 text-sm font-semibold text-white hover:bg-slate-800 transition disabled:opacity-60"
            >
              {saving ? "Kaydediliyor..." : "Kaydet"}
            </button>

            <p className="text-[11px] text-slate-500">
              Artık Supabase veritabanına kaydediliyor.
            </p>
          </div>
        </div>
      </div>
