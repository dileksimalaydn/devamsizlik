...
export default function CourseCatalogPicker({ open, onClose, onManualFallback, onAdd }: Props) {
  ...
  const handleAdd = async () => {
    ...
    const { value: parsedLimit, error: limitError } = parseCustomLimitInput(customLimit);
    if (limitError) {
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <div>
                <label className="mb-1 block text-xs font-semibold text-foreground">
                  ...
                  <span className="ml-1 font-normal text-muted-foreground">
                    — boş bırakırsan {isMixedType ? "her gün kendi yüzdesine göre ayrı" : "otomatik"} hesaplanır
                  </span>
                </label>
                <input
                  type="number"
